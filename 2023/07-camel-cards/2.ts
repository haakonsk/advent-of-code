...
readLinesSplit('input.txt').forEach(([hand, bid]) => {
  ...
  hands.push({
    cards: cards,
    bid: parseInt(bid),
    numWildCards: cards.filter((card) => card === 'J').length,
  });
});
...
function getHandType(hand: Hand): HandType {
  ...
  getBestHand(hand.cards).forEach((card) => {
...
function getHandValue(hand: Hand): number {
  const cards = getCardValues(hand.cards);
...
function getBestHand(cards: string[]): string[] {
  ...
  let bestHand = cards;
  ['2', '3', '4', '5', '6', '7', '8', '9', 'T', 'Q', 'K', 'A'].forEach((card) => {
    const handType = getHandType({ cards: replaceJokers(cards, card) } as Hand);
    if (handType >= bestHandType) {
      bestHandType = handType;
      bestHand = replaceJokers(cards, card);
    }
  })
...
function replaceJokers(cards: string[], card: string): string[] {
  return cards.join('').replace(/J/g, card).split('');
}

function getCardValues(cards: string[]): number[] {
